...
shc -r -f add-ss && rm add-ss && rm add-ss.x.c && mv add-ss.x add-ss && chmod +x add-ss
shc -r -f add-ssr && rm add-ssr && rm add-ssr.x.c && mv add-ssr.x add-ssr && chmod +x add-ssr
shc -r -f add-tr && rm add-tr && rm add-tr.x.c && mv add-tr.x add-tr && chmod +x add-tr
shc -r -f add-sstp && rm add-sstp && rm add-sstp.x.c && mv add-sstp.x add-sstp && chmod +x add-sstp
shc -r -f add-pptp && rm add-pptp && rm add-pptp.x.c && mv add-pptp.x add-pptp && chmod +x add-pptp
shc -r -f add-l2tp && rm add-l2tp && rm add-l2tp.x.c && mv add-l2tp.x add-l2tp && chmod +x add-l2tp
shc -r -f trial && rm trial && rm trial.x.c && mv trial.x trial && chmod +x trial
shc -r -f status && rm status && rm status.x.c && mv status.x status && chmod +x status
shc -r -f clear-log && rm clear-log && rm clear-log.x.c && mv clear-log.x clear-log && chmod +x clear-log
