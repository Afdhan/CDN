...
until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${user_EXISTS} == '0' ]]; do
		...
		user_EXISTS=$(grep -w $user /etc/go-go/akun.conf | wc -l)
...
sed -i '/"'""$uuid""'"$/a\,"'""$user""'"' /etc/go-go/config.json
...
echo -e "### $user $exp" >> /etc/go-go/akun.conf
systemctl restart trojan-go
trojanlink="trojan-go://${uuid}@IP_BUG:${tr}/?sni=${domain}&type=ws&host=${domain}&path=/go-go&encryption=none#${user}"
clear
echo -e ""
echo -e "${cyan}=================================${off}"
